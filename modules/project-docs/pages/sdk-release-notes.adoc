...
This version refines the sub-document counter methods, and adds a new connection string parameter for disabling TLS certificate verification.
...
Added an `MutateInSpec.addAndGet` method that takes a delta.
