...
:page-toclevels: 2
:anyreplica-api: pass:q[https://docs.couchbase.com/sdk-api/couchbase-kotlin-client/kotlin-client/com.couchbase.client.kotlin/-collection/get-any-replica.html]
...
// 
// It can be seen built at http://docs.couchbase.com/kotlin-sdk/3.9/concept-docs/data-durability-acid-transactions.html
...
IMPORTANT: Preferred Server Group Replica Reads are only accessible with the {name-sdk} working with Couchbase Server 7.6.2 or newer (Capella or self-managed), from SDK version 1.4.8.
...
From Kotlin SDK 1.2.3 with Capella (and self-managed Server from 7.6.0), setting a `maxExpiry` of `-1` on a collection will prevent it from expiring, even when its containing bucket has reached expiry time.
