...
If you don't do this, `row.explanation` will be an empty ByteArray.
...
+
NOTE: If all requested fields are missing or unstored, and you're not searching a multi-collection index, `row.fields` will be null and `row.fieldsAs<T>` will return null.
...
----
<1> The server will only search in these collections
...
By default, the FTS service will not wait.
It will only search documents that were already indexed when the search started.
...
In other words, the search results will be "consistent with" the changes you made.
...
If there is a problem with a partition, the FTS service will give you results from only the healthy partitions.
