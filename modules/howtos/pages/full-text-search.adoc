= Search
:description: You can use the Search Service to find JSON documents that have certain words, phrases, or geographic coordinates -- and for vector searches against Server 7.6.
:page-topic-type: howto
:page-aliases: full-text-searching-with-sdk.adoc
:page-toclevels: 2
...
The Search Service finds JSON documents that have certain words, phrases, or geographic coordinates.
...
For all kinds of Search Service searches, you can ask the server to count the number of matching documents that belong to different categories, called "facets."
...
[#simple-example]
== A Simple Search Service Example
...
The Search Service can do xref:{version-server}@server:fts:fts-supported-queries.adoc[many kinds of queries].
...
By default, a `SearchRow` only has a document ID, a score, and the name of the Search Service index partition it came from.
...
Suppose you have a scoped index called `vector-index`, and this index says the document field named `vector_field` contains a vector (an array of numbers).
The following examples show different ways to do vector searches on this field.

CAUTION: The following examples use `cluster/scope.search()` and `SearchSpec`.
These bits of the SDK are currently experimental and may change without notice.
...
[source,kotlin]
----
include::example$FullTextSearch.kt[tag=singleVector,indent=0]
----
<1> This happens to be a scoped index, so we are using `scope.search()`.
If it was a global index we would use `cluster.search()` instead.
See <<Scoped vs Global Indexes>>.
...
<3> The `numCandidates` parameter affects how many results are returned.
It is optional, and defaults to 3.
<4> The `search` method returns the same `Flow<SearchFlowItem>` described earlier.
Nothing happens until you collect the flow.
Calling `execute` is an easy way to collect the flow and turn it into a `SearchResult`.
...
[source,kotlin]
----
include::example$FullTextSearch.kt[tag=vectorAnyOf,indent=0]
----
<1> `SearchSpec.anyOf` combines the child queries using a logical `OR` operator.
For logical `AND`, use `SearchSpec.allOf` instead.
<2> Vector queries can be boosted just like non-vector queries.
Boost is optional, and defaults to `1.0`.

==== Combining Vector and Non-Vector Search
You can use `SearchSpec.mixedMode` to combine a traditional Search query with vector search.

.Mixed Mode Search
[source,kotlin]
----
include::example$FullTextSearch.kt[tag=mixedMode,indent=0]
----
...
[source,kotlin]
----
include::example$FullTextSearch.kt[tag=traditionalTextual,indent=0]
----
<1> A traditional textual search query.
...
When you change a document in Couchbase, it takes time for the Search Service to index the document.
A Search index "runs behind" the KV service.
When you execute a Search Service, you get to choose if you want to wait for the index to "catch up" to the latest KV changes.

[#scan-consistency-unbounded]
=== Unbounded

By default, the Search Service does not wait.
...
A Search index can have multiple partitions that live on different Couchbase Server nodes.
If there is a problem with a partition, the Search Service gives you the results from only the healthy partitions.
...
If you want to know if the Search Service was able to search all partitions, check the `SearchMetadata.errors` property.
