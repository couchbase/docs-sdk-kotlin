...
TIP: The `query` method returns a `Flow<QueryFlowItem>`.
Nothing happens until you collect the flow.
Calling `execute` is one way to collect the flow.
...
If you forget to open a bucket, the `query` method throws `FeatureNotAvailableException`.
====
...
If you use a parameter where it is not allowed, `execute` throws an exception.
...
This example requests all metadata, and prints it:
...
If you run a query frequently, it's good to ask the server to remember the plan.
...
Sometimes the server can make better plans for an adhoc query than for a prepared statement.
It's good to measure the performance of your query so you know whether it's better to use a prepared statement.
...
When you execute a query, you get to choose if you want to wait for the Query indexes to "catch up" to the latest KV changes.
...
To use "consistent with" scan consistency, you must keep track of the mutation tokens from the changes you want to wait for.
