...
//The Couchbase Kotlin SDK uses https://github.com/FasterXML/jackson-databind[Jackson], a popular and fast library for JSON data binding.
//You can tell the SDK to use a different library.
...
The SDK will use data binding to convert your object to JSON.
...
[CAUTION]
====
If you use a `String` for the content, and don't skip data binding, the SDK will assume you want the document content to be a JSON String.
For example, if you write:
[source,kotlin]
----
include::example$Json.kt[tag=failToSkipDataBinding,indent=0]
----
The document content in Couchbase will be:
[source,json]
----
"{\"favoriteColor\":\"blue\"}"
----
...
The SDK includes serializers for popular JSON processing libraries.
...
.Jackson Tree Model
====
Jackson can represent a JSON document as a tree of `TreeNode` objects.
You can use data binding with `TreeNode`.
...
Sometimes it's easier to do data binding with a `TreeNode` than a `Map`.
====
...
When you write a document to Couchbase, a "transcoder" controls how the `content` argument is converted into the bytes stored in Couchbase.

The default transcoder is called `JsonTranscoder`. It uses a `JsonSerializer` to convert the content to and from JSON.

The SDK knows about three types of content:



There are some other important transcoders.
