...
For example, let's pretend we're writing a program that helps a storyteller remember details about characters in their stories.
...
If you set this parameter, the operation fails with `CasMismatchException` if the document's current CAS value does not match the `cas` you set.
...
Changing a locked document releases the lock.
...
Pessimistic locking is very expensive.
