...
Each node in a Couchbase cluster has its own TLS certificate which has been signed by a Certificate Authority (CA) certificate, also known as the root certificate.
When the SDK makes a connection using TLS, it verifies the node it's talking to has a certificate signed by a trusted CA.

In just a moment, you'll learn how to configure the SDK to trust the CA certificate used by the Couchbase cluster, but first...
...
.Enable TLS using a custom trust manager factory.
[source,kotlin]
----
...
This is insecure.
Don't do it.
Like, _ever_, not even if it's "just for development."
It will slip into your production code by mistake, and you'll be very sad.
In fact, forget you ever saw this.
...
The SDK gives you the power to do dangerous things like disable hostname verification, or even disable all security checks by using an insecure trust manager factory.
